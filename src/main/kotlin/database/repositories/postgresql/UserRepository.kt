...
class UserRepository: IUserRepository {
    override fun getUser(userId: Int): UserEntity = dbQuery {
        UserEntity.findById(userId) ?: throw NotFoundException("User with id $userId not found")
    }

    override fun getUser(username: String): UserEntity = dbQuery {
        UserEntity.find(UsersTable.username eq username).firstOrNull() ?: throw NotFoundException("User with username $username not found")
    }
}