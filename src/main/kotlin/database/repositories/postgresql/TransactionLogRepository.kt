...
class TransactionLogRepository : ITransactionLogRepository {
    override fun getCardTransactionLog(volunteerIdentifier: Int, page: Int, pageSize: Int): List<TransactionLogDTO> =
        dbQuery {
            TransactionLogsTable
                ...
                .selectAll()
                .where { TransactionLogsTable.cardId eq volunteerIdentifier }
                .limit(count = pageSize.coerceAtLeast(minimumValue = 1))
    ...
    override fun getCardTransactionLog(volunteerIdentifier: String, page: Int, pageSize: Int): List<TransactionLogDTO> =
        dbQuery {
            TransactionLogsTable
                .join(
                    otherTable = CardsTable,
                ...
                .selectAll()
                .where { CardsTable.nfc eq volunteerIdentifier }
                .limit(count = pageSize.coerceAtLeast(minimumValue = 1))
                .offset(start = calculateOffset(page, pageSize))
                .orderBy(column = TransactionLogsTable.date, order = SortOrder.DESC)
                .map { row ->
    ...
    private fun calculateOffset(page: Int, pageSize: Int): Long {
        val pageNumber = page.coerceAtLeast(minimumValue = 1)
        val pageSizeNumber = pageSize.coerceAtLeast(minimumValue = 1)
